...
struct UFoliageStatistics : UBlueprintFunctionLibrary {
	...
	int32_t FoliageOverlappingSphereCount(struct UObject* WorldContextObject, struct UStaticMesh* StaticMesh, struct FVector CenterPosition, float Radius); // Function Foliage.FoliageStatistics.FoliageOverlappingSphereCount // (Final|Native|Static|Public|HasDefaults|BlueprintCallable) // @ game+0x2c06d60
	int32_t FoliageOverlappingBoxCount(struct UObject* WorldContextObject, struct UStaticMesh* StaticMesh, struct FBox Box); // Function Foliage.FoliageStatistics.FoliageOverlappingBoxCount // (Final|Native|Static|Public|HasDefaults|BlueprintCallable) // @ game+0x2c06c00
};
...
struct AInteractiveFoliageActor : AStaticMeshActor {
	...
	struct FVector TouchingActorEntryPosition; // 0x258(0x0c)
	struct FVector FoliageVelocity; // 0x264(0x0c)
	struct FVector FoliageForce; // 0x270(0x0c)
	struct FVector FoliagePosition; // 0x27c(0x0c)
	float FoliageDamageImpulseScale; // 0x288(0x04)
	float FoliageTouchImpulseScale; // 0x28c(0x04)
	float FoliageStiffness; // 0x290(0x04)
	float FoliageStiffnessQuadratic; // 0x294(0x04)
	float FoliageDamping; // 0x298(0x04)
	float MaxDamageImpulse; // 0x29c(0x04)
	float MaxTouchImpulse; // 0x2a0(0x04)
	float MaxForce; // 0x2a4(0x04)
	float Mass; // 0x2a8(0x04)
	char pad_2AC[0x4]; // 0x2ac(0x04)

	void CapsuleTouched(struct UPrimitiveComponent* OverlappedComp, struct AActor* Other, struct UPrimitiveComponent* OtherComp, int32_t OtherBodyIndex, bool bFromSweep, struct FHitResult OverlapInfo); // Function Foliage.InteractiveFoliageActor.CapsuleTouched // (Final|Native|Protected|HasOutParms) // @ game+0x2c069d0
};
...
struct AProceduralFoliageBlockingVolume : AVolume {
	struct AProceduralFoliageVolume* ProceduralFoliageVolume; // 0x278(0x08)
};
...
struct UProceduralFoliageSpawner : UObject {
	...
	void Simulate(int32_t NumSteps); // Function Foliage.ProceduralFoliageSpawner.Simulate // (Final|Native|Public|BlueprintCallable) // @ game+0x2c06ec0
};
...
struct AProceduralFoliageVolume : AVolume {
	struct UProceduralFoliageComponent* ProceduralComponent; // 0x278(0x08)
};
